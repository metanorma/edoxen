...
Each `Localization` declares its `language_code` (ISO 639-3 three-letter
...
* Single file per meeting (no `-en.yaml` / `-fr.yaml` split)
* Translators can compare EN+FR side-by-side in the same file
* Language-agnostic fields (identifier, doi) declared once, not duplicated
* Forward-compatible: adding a third language is one new entry per
...
A monolingual rendering of a Resolution.

`language_code`:: ISO 639-3 three-letter language code (e.g. `eng`,
  `fra`, `deu`, `spa`, `jpn`, `rus`, `ara`). Pattern: `^[a-z]{3}$`.
`script`:: ISO 15924 four-letter script code (e.g. `Latn`, `Cyrl`,
  `Hant`, `Arab`). Pattern: `^[A-Z][a-z]{3}$`.
`title`:: Localized resolution title.
`subject`:: Localized subject or scope.
`message`:: Localized resolution message (optional).
`considering`:: Localized considering clause (optional).
`considerations`:: Localized considerations (optional).
`actions`:: Localized actions.
`approvals`:: Localized approvals (optional).

=== SourceUrl

A per-language source-PDF reference used in `metadata.source_urls[]`.

`ref`:: The URL string.
`format`:: Document format (e.g. `pdf`, `html`).
`language_code`:: ISO 639-3 code for the language of this URL's
  content.
