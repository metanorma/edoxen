  ...
  class SchemaValidator
    ...
    def validate_file(file_path)
      unless File.exist?(file_path)
        return [ValidationError.new(file_path, 1, 1, "File not found")]
      end
    ...
    def build_line_map(content)
      ...
      lines.each_with_index do |line, index|
        ...
        if stripped.start_with?("- ")
          ...
          item_content = stripped[2..-1].strip
          ...
          array_indices.keys.each do |path|
    ...
    def find_line_for_pointer(pointer, line_map)
      ...
      if parts.length >= 4 && parts[0] == "resolutions" && parts[2] == "actions"
        ...
        property = parts[4] if parts.length > 4
      ...
      base_message = case type
                     ...
                     when "additionalProperties"
                       ...
                       if extra.length == 1
                         ...
                         "object properties #{extra.map { |k| "'#{k}'" }.join(", ")} are disallowed additional properties"
                       end
                     when "enum"
                       value = details["value"]
